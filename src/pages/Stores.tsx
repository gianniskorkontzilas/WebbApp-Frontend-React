...
// Stores.tsx
import React, { useState, useEffect } from "react";
import { Button, CircularProgress, Box, Typography, Card, CardContent, CardActions, Dialog, DialogActions, DialogContent, DialogTitle, TextField, IconButton } from "@mui/material";
import { Delete } from "@mui/icons-material";
import { Link, useNavigate, useLocation } from "react-router-dom";
import axiosInstance from "../api/axiosInstance.ts";
...
const Stores: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStores = async () => {
      try {
        ...
        console.error("Error fetching stores:", error);
        setError("Error fetching stores");
        showError("Error fetching stores", "error");  
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDelete = async (storeId: string) => {
    try {
      await axiosInstance.delete(`/stores/${storeId}`);
      setStores(stores.filter(store => store.id !== storeId));
      showError("Store deleted successfully.", "success");  
    } catch (error) {
      console.error("Error deleting store:", error);
      showError("Failed to delete store.", "error");  
    }
  };
