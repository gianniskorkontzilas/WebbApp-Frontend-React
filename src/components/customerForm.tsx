...
const CustomerForm = () => {
    ...
    const [vatNumber, setVatNumber] = useState("");
    const [dateOfBirth, setDateOfBirth] = useState("");
    const [storeId, setStoreId] = useState<number | null>(null); // Προσθήκη storeId στο state
    const [snackbar, setSnackbar] = useState({ open: false, message: "", severity: "success" });
    ...
    useEffect(() => {
        // Αν υπάρχει storeId στο URL, το μετατρέπουμε σε αριθμό
        if (storeIdParam) {
        ...
        if (customerId) {
            const fetchCustomer = async () => {
                try {
                    ...
                    setVatNumber(response.data.vatNumber);
                    setDateOfBirth(response.data.dateOfBirth);
                    setStoreId(response.data.storeId); // Ορισμός storeId από την απόκριση του backend
                } catch (error) {
                    console.error("Error fetching customer:", error);
                    showSnackbar("Failed to fetch customer details.", "error");
                }
